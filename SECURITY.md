...
- **Safety rails on every mutation:**
  ...
  - Applies are **idempotent** — a no-change re-run produces the same file and
    zero `apply.change` audit entries.
  - Every apply, backup, and rollback is appended to a tamper-evident
    `/var/log/onionarmor/audit.log` with timestamp, operator, and details.

## Reporting a vulnerability

Report security issues privately to **security@1aeo.com**. Please include:
...
> The `security@1aeo.com` address is a placeholder until a dedicated security
