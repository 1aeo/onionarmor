...
@test "revert: ONIONARMOR_SKIP_RELOAD reports runtime untouched, not falsely cleared" {
  # Regression + branch coverage: the summary must not claim 'cleared' when we
  # skipped the reset, AND — by design — ONIONARMOR_SKIP_RELOAD means "don't
  # touch the live kernel at all", so revert runs NEITHER `sysctl -w …=` NOR
  # `sysctl --system` (symmetric with apply, which skips the load under the same
  # knob). This deliberately differs from "gate only --system, always run -w".
  seed_metrics_fleet 48010 48050
  bash "$APPLY" --auto >/dev/null
  printf '48010-48050\n' > "$ONIONARMOR_KRP_PROC_FILE"
  : > "$STUB_SYSCTL_LOG"
  ...
  [ ! -e "$DROPIN" ]
  # No live-kernel mutation under SKIP_RELOAD.
  ! grep -q -- '-w net.ipv4.ip_local_reserved_ports=' "$STUB_SYSCTL_LOG"
  ! grep -q -- '--system' "$STUB_SYSCTL_LOG"
}
