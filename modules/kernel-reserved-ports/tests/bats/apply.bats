...
@test "test_listen_ip_filter: non-loopback port is excluded by default" {
  seed_instance relay1 "MetricsPort 127.0.0.1:48001" "SocksPort 203.0.113.190:443"
  run bash "$APPLY" --auto
  [ "$status" -eq 0 ]
...
@test "apply --auto with no loopback tor ports: fails clearly" {
  seed_instance relay1 "ORPort 203.0.113.190:9001"
  run bash "$APPLY" --auto
  [ "$status" -ne 0 ]
...
@test "apply: reads /etc/tor/torrc.all and /run/tor-instances/*.defaults" {
  ...
  printf 'ControlPort 127.0.0.1:29100\n' > "$ONIONARMOR_KRP_TOR_RUN_DIR/instance-1.defaults"
  run bash "$APPLY" --auto
  [ "$status" -eq 0 ]
